...
// #define TO_MB(b) ((b) / 1048576.0) // 1MB = 1024*1024 bytes
...
unsigned long freeMem, totalMem; // 使用long类型避免溢出
...
int main(int argc, char* argv[])
{
    ...
    if (!(argc >= 2 && !strcmp(argv[1], "-r")))
    {
        ...
        SSD1306_PutString(21, 2, "OrangePi Zero 3", MF_6x8, White);
        SSD1306_PutString(15, 16, "LFNL", MF_16x26, White);
...
void Work()
{
    ...
    /*显示内存利用率*/
    freeMem = GetMemAvailable();  // 单位：kB
    totalMem = GetMemTotal();     // 单位：kB

    memset(tempStr, 0, TEMP_STR_LEN);
    sprintf(tempStr, "Mem:%.1fMB %d%%", 
            (totalMem - freeMem)/1024.0,  // 直接kB转MB
            (int)((totalMem - freeMem) * 100.0 / totalMem));
